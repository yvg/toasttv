...
import type { ConfigRepository, AppConfig } from '../src/repositories/ConfigRepository'
import type {
  IDateTimeProvider,
  MediaItem,
} from '../src/types'
...
function buildAppConfig(overrides: Partial<DeepPartial<AppConfig>> = {}): AppConfig {
  return {
...
function buildMockRepository(
  ...
  return {
    ...
    getById: mock((id: number) => Promise.resolve(allMedia.find(m => m.id === id) ?? null)),
...
function createEngine(
  ...
  dateTime: IDateTimeProvider = new FakeDateTimeProvider(new Date('2024-01-01T10:00:00'))
): PlaylistEngine {
...
describe('PlaylistEngine', () => {
  test('startSession returns intro video when configured', async () => {
    const introVideo = buildMediaItem({ id: 99, filename: 'intro.mp4', mediaType: 'intro' })
  ...
  test('getNextVideo plays interlude at configured frequency', async () => {
    ...
    const interludes = [
      buildMediaItem({ id: 100, filename: 'interlude.mp4', isInterlude: true, durationSeconds: 30 }),
    ]
  ...
  test('session expires after limit minutes', async () => {
    ...
    const showVideo = buildMediaItem({ id: 1, filename: 'show.mp4', durationSeconds: 600 }) // 10min
    const outroVideo = buildMediaItem({ id: 88, filename: 'outro.mp4', mediaType: 'outro', durationSeconds: 60 })
    
    // Include outro in cache so findCachedItem works
    const repo = buildMockRepository([showVideo, outroVideo])
    repo.getById = mock((id) => Promise.resolve(id === 88 ? outroVideo : id === 1 ? showVideo : null))
  ...
  test('getNextVideo avoids recently played', async () => {
    ...
    const repo = buildMockRepository(videos)
    const config = buildMockConfig(buildAppConfig({
