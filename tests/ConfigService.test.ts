...
describe('ConfigService', () => {
  ...
  test('setSessionLimit() validates positive number', async () => {
    ...
    expect(mockRepo.update).toHaveBeenCalledWith({ session: { limitMinutes: 45 } })
  })
  ...
  test('setSessionLimit() allows zero (infinite)', async () => {
    ...
    expect(mockRepo.update).toHaveBeenCalledWith({ session: { limitMinutes: 0 } })
  })
  ...
  test('setSessionLimit() rejects negative', async () => {
    ...
    await expect(service.setSessionLimit(-5)).rejects.toThrow('Session limit cannot be negative')
  })
  ...
  test('setInterludeConfig() rejects frequency < 1', async () => {
    ...
    await expect(service.setInterludeConfig(true, 0)).rejects.toThrow('Interlude frequency must be at least 1')
  })
  ...
  test('setLogoConfig() enables logo with settings', async () => {
    ...
    expect(mockRepo.update).toHaveBeenCalledWith({
      logo: { imagePath: '/path/to/logo.png', opacity: 150, position: 3, enabled: true }
    })
  })
